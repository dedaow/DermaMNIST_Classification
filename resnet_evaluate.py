...
def evaluate_on_test_set(test_dataset, model_path, num_classes=7, batch_size=32):
    """
    Evaluate the ResNet-18 model on the test set, report accuracy, precision, recall, F1-score, 
    plot ROC curve, compute AUC, visualize confusion matrix, and generate combined attention maps.
    ...
    plt.savefig("eval/resnet/roc_curve.png")
    plt.show()
    ...
    plt.savefig("eval/resnet/confusion_matrix.png")
    plt.show()
    ...
    for label, maps in attention_maps.items():
        ...
        plt.savefig(f"eval/resnet/resnet_combined_attention_maps/resnet_class_{label}.png")
        plt.show()

    # Save the results
    with open("eval/resnet/resnet18_evaluation_testing.txt", "w") as f:
    ...
    print("Test results saved to eval/resnet/resnet18_evaluation_testing.txt")

# Example usage
train_dataset, val_dataset, test_dataset = data_loader()
evaluate_on_test_set(test_dataset, model_path="model/resnet18_checkpoint.pth")
